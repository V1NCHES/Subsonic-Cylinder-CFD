...
\begin{figure}[h!]\centering\includegraphics[width=0.4\textwidth]{1_physical_setup.png}\caption{Физическая постановка}\label{fig:setup}\end{figure}
	...
	\begin{enumerate}
		...
		\item Составить разностную схему и решить ее с помощью ПНМ (Полностью Неявный Метод), сравнить данные по сходимости метода ПНМ с методом строчной релаксации (SLOR).
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{2_conformal_mapping_new.png}
	\caption{Схема расчётной области в координатах $(\xi, \eta)$. Левая граница ($\xi=0$) соответствует поверхности цилиндра (твердая стенка), правая — бесконечности.}
	\label{fig:mapping}
\end{figure}
	...
	\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{stencil_scheme.png}
...
\section{Полностью неявный метод (ПНМ)}
	...
	\subsection{Теоретические основы и метод SIP}
	
	\subsubsection{Формулировка разностного уравнения}
	Для внутреннего узла $(i,j)$, где $i = 1,\dots,N_\xi-1$, $j = 1,\dots,N_\eta-1$, разностное уравнение запишем в виде $F_{i,j} = 0$:
	\begin{equation}
		\begin{aligned}
			F_{i,j} = &\frac{1}{\Delta \xi^2} \left[ \rho_{i+\frac{1}{2},j} (\Phi_{i+1,j} - \Phi_{i,j}) - \rho_{i-\frac{1}{2},j} (\Phi_{i,j} - \Phi_{i-1,j}) \right] + \\
			&+ \frac{1}{\Delta \eta^2} \left[ \rho_{i,j+\frac{1}{2}} (\Phi_{i,j+1} - \Phi_{i,j}) - \rho_{i,j-\frac{1}{2}} (\Phi_{i,j} - \Phi_{i,j-1}) \right] = 0,
		\end{aligned}
		\label{eq:F}
	\end{equation}
	...
	Пусть $\boldsymbol{\Phi}^{(k)}$ — приближенное решение на $k$-й итерации. Ищем поправку $\delta \boldsymbol{\Phi} = \boldsymbol{\Phi}^{(k+1)} - \boldsymbol{\Phi}^{(k)}$ такую, что:
	\[
	F_{i,j}(\boldsymbol{\Phi}^{(k)} + \delta \boldsymbol{\Phi}) = 0.
	\]
	Разлагая в ряд Тейлора с сохранением только линейных членов:
	\[
	F_{i,j}^{(k)} + \sum_{m,n} \frac{\partial F_{i,j}^{(k)}}{\partial \Phi_{m,n}} \delta \Phi_{m,n} = 0, \text{ где } F_{i,j}^{(k)} = F_{i,j}(\boldsymbol{\Phi}^{(k)}).
	\label{eq:newton_linear}
	\]

	
	В силу пятиточечной структуры схемы, $F_{i,j}$ зависит только от пяти узлов: $\Phi_{i-1,j}$, $\Phi_{i,j}$, $\Phi_{i+1,j}$, $\Phi_{i,j-1}$, $\Phi_{i,j+1}$. Поэтому уравнение (\ref{eq:newton_linear}) принимает вид:
	\begin{equation}
		\begin{aligned}
			& \frac{\partial F_{i,j}^{(k)}}{\partial \Phi_{i-1,j}} \delta \Phi_{i-1,j} + \frac{\partial F_{i,j}^{(k)}}{\partial \Phi_{i,j}} \delta \Phi_{i,j} + \frac{\partial F_{i,j}^{(k)}}{\partial \Phi_{i+1,j}} \delta \Phi_{i+1,j} +\\
			&+ \frac{\partial F_{i,j}^{(k)}}{\partial \Phi_{i,j-1}} \delta \Phi_{i,j-1} + \frac{\partial F_{i,j}^{(k)}}{\partial \Phi_{i,j+1}} \delta \Phi_{i,j+1} = - F_{i,j}^{(k)},
		\end{aligned}
		\label{eq:newton_system}
	\end{equation}
	
	\subsubsection{Вычисление производных (элементов матрицы Якоби)}
	...
	\[
	\rho = \rho(q^2), \quad q^2 = e^{-2\xi_i} \left[ \left( \frac{\partial \Phi}{\partial \xi} \right) ^2 + \left( \frac{\partial \Phi}{\partial \eta} \right) ^2 \right].
	\]
	Производные $\frac{\partial \rho}{\partial \Phi}$ вычисляются как сложная функция:
	\[
	\frac{\partial \rho}{\partial \Phi} = \frac{d\rho}{dq^2} \cdot \frac{\partial q^2}{\partial \Phi}.
	\]
	...
	\paragraph{Замечание о линеаризации:} В полном методе Ньютона необходимо учитывать зависимость плотности от потенциала через вышеприведенные производные. Однако в практических расчетах (включая представленный код) часто используется метод Пикара (последовательные приближения), где плотность в матрице Якоби считается «замороженной» на текущем итерационном слое ($\partial \rho / \partial \Phi \approx 0$). Это существенно упрощает вид коэффициентов матрицы и ускоряет одну итерацию без потери точности сошедшегося решения. Ниже приведены коэффициенты именно для этого случая.
	
	\subsubsection{Формирование СЛАУ в терминах SIP}
	
	Для применения метода Стоуна систему линейных уравнений (\ref{eq:newton_system}) запишем в общем пятиточечном виде:
	\begin{equation}
		B_{i,j} \delta \Phi_{i,j-1} + D_{i,j} \delta \Phi_{i-1,j} + E_{i,j} \delta \Phi_{i,j} + F_{i,j} \delta \Phi_{i+1,j} + H_{i,j} \delta \Phi_{i,j+1} = q_{i,j},
	...
	\begin{itemize}
		\item $B_{i,j} = \rho_{i,j-1/2} / \Delta \eta^2$ (Юг),
		\item $D_{i,j} = \rho_{i-1/2,j} / \Delta \xi^2$ (Запад),
		\item $F_{i,j} = \rho_{i+1/2,j} / \Delta \xi^2$ (Восток),
		\item $H_{i,j} = \rho_{i,j+1/2} / \Delta \eta^2$ (Север),
		\item $E_{i,j} = -(B_{i,j} + D_{i,j} + F_{i,j} + H_{i,j})$ (Центр).
	\end{itemize}
	...
	\begin{enumerate}
		...
		Используется условие симметрии $\Phi_{-1,j} = \Phi_{0,j}$. Разностное уравнение (\ref{eq:sip_stencil}) для узлов $i=0$ принимает вид:
		\[ E_{0,j} \delta\Phi_{0,j} + F_{0,j} \delta\Phi_{1,j} + B_{0,j} \delta\Phi_{0,j-1} + H_{0,j} \delta\Phi_{0,j+1} = q_{0,j}, \]
		...
		Используется условие $\Phi_{i,-1} = \Phi_{i,0}$. Уравнение для узлов $j=0$ (при $i > 0$):
		\[ E_{i,0} \delta\Phi_{i,0} + D_{i,0} \delta\Phi_{i-1,0} + F_{i,0} \delta\Phi_{i+1,0} + H_{i,0} \delta\Phi_{i,1} = q_{i,0}, \]
		...
		Поскольку на бесконечности задано фиксированное значение потенциала (условие Дирихле), поправка в этих узлах должна быть равна нулю: $\delta \Phi_{N_\xi,j} = 0$. Это достигается установкой следующих коэффициентов:
		\[ E_{N_\xi,j} = 1, \quad B = D = F = H = 0, \quad q_{N_\xi,j} = 0. \]
	\end{enumerate}



	В глобальном векторе неизвестных система уравнений принимает вид $A \delta \boldsymbol{\Phi} = \mathbf{q}$, где матрица $A$ имеет характерную пятидиагональную структуру:
	\[
	A = \begin{pmatrix}
		E & F & 0 & \dots & H & & 0 \\
		D & E & F & \ddots & 0 & \ddots & \\
	...
	Метод Стоуна \cite{stone1968} (Strongly Implicit Procedure) использует модифицированное неполное $LU$-разложение матрицы системы: $[A+N] \delta \boldsymbol{\Phi} = \mathbf{q}$, где матрица $N$ выбирается таким образом, чтобы аппроксимируемая система $[A+N]$ была близка к исходной $A$ и легко факторизовалась.
	...
	После вычисления коэффициентов решение $\delta \Phi_{i,j}$ находится в два этапа:
	\begin{enumerate}
		\item \textbf{Прямой ход ($L\mathbf{y} = \mathbf{q}$):}
		\[ y_{i,j} = (q_{i,j} - b_{i,j} y_{i,j-1} - c_{i,j} y_{i-1,j}) / d_{i,j}. \]
		\item \textbf{Обратный ход ($U \delta \boldsymbol{\Phi} = \mathbf{y}$):}
		\[ \delta \Phi_{i,j} = y_{i,j} - e_{i,j} \delta \Phi_{i+1,j} - f_{i,j} \delta \Phi_{i,j+1}. \]
	\end{enumerate}
	
	Итерационный процесс продолжается до обеспечения малости поправки $\|\delta \Phi\|_\infty < \epsilon$. Преимущество метода SIP заключается в его высокой скорости сходимости и устойчивости для эллиптических задач на сильно вытянутых сетках.
	...
	\begin{enumerate}
		...
		\begin{itemize}
			\item Задание расчетной сетки и начального приближения потенциала $\Phi^{(0)}$ (например, решение для несжимаемой жидкости).
			\item Установка граничных значений на бесконечности ($i=N_\xi$) согласно асимптотике.
		\end{itemize}
		
		\item \textbf{Итерационный цикл (нахождение поправки $\delta\Phi$):}
		\begin{enumerate}
			\item Расчет поля плотности $\rho$ на основе текущих градиентов потенциала.
			\item Формирование СЛАУ для поправок: расчет невязок $q_{i,j}$ и коэффициентов пятиточечного шаблона $B, D, E, F, H$ с учетом граничных условий.
			\item \textbf{Решение СЛАУ методом SIP (Стоуна):}
			\begin{itemize}
				\item Прямой ход: вычисление промежуточного вектора $\mathbf{y}$ с использованием матриц неполного разложения.
				\item Обратный ход: последовательное нахождение поправок $\delta\Phi_{i,j}$ для всей области.
			\end{itemize}
		\end{enumerate}
		
		\item \textbf{Обновление и контроль:}
		\begin{itemize}
			\item Глобальное обновление потенциала: $\Phi \leftarrow \Phi + \delta \Phi$.
			\item Проверка сходимости по норме поправки $\|\delta \Phi\|_\infty < \epsilon$. В случае невыполнения — переход к следующей итерации.
		\end{itemize}
	\end{enumerate}
	...
	\begin{itemize}
		...
		\item $D_i = \displaystyle - \frac{\rho_{i,j+1/2} \Phi_{i,j+1} + \rho_{i,j-1/2} \Phi_{i,j-1}}{\Delta \eta^2}$ — правая часть, содержащая вклады от соседних строк.
	\end{itemize}
	...
	\begin{enumerate}
		\item \textbf{Левая граница ($i=0$, поверхность цилиндра):} 
		С использованием условия $\Phi_{-1,j} = \Phi_{0,j}$ уравнение для первого узла строки принимает вид:
		\[ B_0 \Phi_{0,j}^{(n+1)} + C_0 \Phi_{1,j}^{(n+1)} = D_0, \]
		где коэффициенты:
		\[ B_0 = -\left( \frac{\rho_{1/2,j}}{\Delta \xi^2} + \frac{\rho_{0,j+1/2} + \rho_{0,j-1/2}}{\Delta \eta^2} \right), \quad C_0 = \frac{\rho_{1/2,j}}{\Delta \xi^2}. \]
		
		\item \textbf{Правая граница ($i=N_\xi$, бесконечность):} 
		Значение потенциала фиксировано: $\Phi_{N_\xi,j} = \Phi_{N_\xi,j}^\infty$. Это значение входит в правую часть уравнения для узла $i=N_\xi-1$:
		\[ A_{N_\xi-1} \Phi_{N_\xi-2,j}^{(n+1)} + B_{N_\xi-1} \Phi_{N_\xi-1,j}^{(n+1)} = D_{N_\xi-1} - C_{N_\xi-1} \Phi_{N_\xi,j}^\infty. \]
		
		\item \textbf{Нижняя и верхняя границы ($j=0, N_\eta$):} 
		За счет условий симметрии по $\eta$ ($\Phi_{i,-1} = \Phi_{i,0}$) правая часть $D_i$ для граничных строк содержит только вклад от внутренней соседней строки:
		\begin{itemize}
			\item При $j=0$: $D_i = \displaystyle - \frac{\rho_{i,1/2} \Phi_{i,1}}{\Delta \eta^2}$,
			\item При $j=N_\eta$: $D_i = \displaystyle - \frac{\rho_{i,N_\eta-1/2} \Phi_{i,N_\eta-1}}{\Delta \eta^2}$.
		\end{itemize}
	\end{enumerate}
	...
	После нахождения решения системы (\ref{eq:slor_tridiag}), обозначим его $\tilde{\Phi}_{i,j}$. Окончательное значение в узле вычисляется с использованием параметра сверхрелаксации $\omega$:
	\begin{equation}
		\Phi_{i,j}^{(n+1)} = (1 - \omega) \Phi_{i,j}^{(n)} + \omega \tilde{\Phi}_{i,j}
	\end{equation}
	...
	\begin{enumerate}
		\item \textbf{Подготовка:} Инициализация потенциала $\Phi$ и расчет начального поля плотности $\rho$.
		...
		\begin{itemize}
			...
			\begin{enumerate}
				...
				\item \textbf{Решение прогонкой (алгоритм Томаса):} получение промежуточного значения $\tilde{\Phi}$ вдоль всей строки.
				\item \textbf{Сверхрелаксация:} $\Phi^{(n+1)} = (1 - \omega) \Phi^{(n)} + \omega \tilde{\Phi}$.
			\end{enumerate}
			\item Пересчет поля плотности $\rho$ после завершения прохода по всем строкам.
		\end{itemize}
		\item \textbf{Критерий остановки:} Расчет нормы невязки или максимальной поправки по всему полю: $\|\delta \Phi\|_\infty < \epsilon$.
	\end{enumerate}
...
%\end{document}	
	...
	В данном разделе представлены результаты расчетов для чисел Маха набегающего потока $M_\infty = 0.1, 0.2, 0.3, 0.4$. Расчеты проводились на сетке $120 \times 90$.

	\subsection{Параметры потока и сходимость стандартных методов}

	В таблице \ref{tab:summary} приведены значения локального числа Маха и коэффициента давления в верхней точке цилиндра ($\theta = 90^\circ$), а также количество итераций, потребовавшихся для сходимости стандартных методов ПНМ (SIP) и SLOR до точности $10^{-7}$.

	\begin{table}[H]
		\centering
		\caption{Сводная таблица результатов: стандартные методы ($120 \times 90$, $\epsilon=10^{-7}$)}
		\label{tab:summary}
		\begin{tabular}{ccccccc}
			\toprule
			$M_\infty$ & $M_{loc}$ (ПНМ) & $M_{loc}$ (SLOR) & $C_p$ (ПНМ) & $C_p$ (SLOR) & Итер. ПНМ & Итер. SLOR \\
			\midrule
			0.1 & 0.1927 & 0.1927 & -2.6771 & -2.6771 & 1412 & 1316 \\
			0.2 & 0.3951 & 0.3951 & -2.7365 & -2.7365 & 1450 & 1333 \\
			0.3 & 0.6219 & 0.6220 & -2.8560 & -2.8563 & 1412 & 1365 \\
			0.4 & 0.9122 & 0.9122 & -3.1119 & -3.1119 & 1550 & 1420 \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{figure}[H]
		...
		\caption{История сходимости $\|\delta \phi\|_\infty$ для ускоренного ПНМ и метода SLOR (с указанием итераций)}
		\label{fig:conv_all}
	\end{figure}
